Public Class Form3
    ...
    Private Sub hitungButton_Click(sender As Object, e As EventArgs) Handles hitungButton.Click
        Dim list As List(Of List(Of Integer)) = Me.parseMatrix(Me.matriks1Text.Text)
        Dim daftar As List(Of List(Of Integer)) = Me.parseMatrix(Me.matriks2Text.Text)
        If ((Not list Is Nothing) And (Not daftar Is Nothing)) Then
            If Me.isValidMatrix(list, daftar) Then
                Select Case Convert.ToString(Me.operasiCombo.SelectedItem)
                    Case "Tambah"
                        list = Me.tambah(list, daftar)
                        Me.print(list)
                        Exit Select
                    Case "Kurang"
                        list = Me.kurang(list, daftar)
                        Me.print(list)
                        Exit Select
                End Select
            Else
                MessageBox.Show("Jumlah baris dan kolom matriks harus sama!", "Peringatan", MessageBoxButtons.OK, MessageBoxIcon.Hand)
            End If
        Else
