Public Class Form2
    ...
    Private Sub BtnOk_Click(sender As Object, e As EventArgs) Handles BtnOk.Click
        ...
        If input <> "0" Then
            Select Case Convert.ToString(Me.Cbpilih.SelectedItem)
                Case "Pyramid"
                    Me.txthasil.Text = Me.pyramid(input)
                    Exit Select
                Case "Hollow Pyramid"
                    Me.txthasil.Text = Me.hollowPyramid(input)
                    Exit Select
                Case "Inverted Pyramid"
                    Me.txthasil.Text = Me.invertedPyramid(input)
                    Exit Select
                Case "Inverted Hollow Pyramid"
                    Me.txthasil.Text = Me.invertedHollowPyramid(input)
                    Exit Select
                Case Nothing
                    MessageBox.Show("Silahkan pilih pola!", "Informasi", MessageBoxButtons.OK, MessageBoxIcon.Asterisk)
                    Exit Select
            End Select
        Else
            MessageBox.Show("Silahkan masukkan angka!", "Informasi", MessageBoxButtons.OK, MessageBoxIcon.Asterisk)
        End If
    End Sub
